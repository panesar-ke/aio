import { pgTable, foreignKey, serial, date, integer, uuid, boolean, varchar, text, numeric, bigserial, timestamp, uniqueIndex, index, unique, bigint, primaryKey, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const aalLevel = pgEnum("aal_level", ['aal3', 'aal2', 'aal1'])
export const accountStateEnum = pgEnum("accountStateEnum", ['lead', 'account'])
export const appraisalDetailTypeEnum = pgEnum("appraisalDetailTypeEnum", ['OBJECTIVE', 'FINAL'])
export const appraisalStatusEnum = pgEnum("appraisalStatusEnum", ['NOT SET', 'PENDING', 'STAFF FILLED', 'CONDUCTED', 'COMPLETED', 'FILLED'])
export const bloodTypeEnum = pgEnum("bloodTypeEnum", ['A', 'B', 'AB', 'O'])
export const codeChallengeMethod = pgEnum("code_challenge_method", ['plain', 's256'])
export const disciplinaryCaseActionEnum = pgEnum("disciplinaryCaseActionEnum", ['VERBAL-WARNING', 'WRITTEN-WARNING', 'SURCHARGE', 'SUSPENSION', 'TERMINATION', 'OTHER'])
export const disciplinaryCaseStatusEnum = pgEnum("disciplinaryCaseStatusEnum", ['REPORTED', 'UNDER-INVESTIGATION', 'RESOLVED', 'CLOSED'])
export const discountTypeEnum = pgEnum("discountTypeEnum", ['NONE', 'PERCENTAGE', 'AMOUNT'])
export const educationLevelEnum = pgEnum("educationLevelEnum", ['PHD', 'MASTERS-DEGREE', 'BACHELORS-DEGREE', 'HIGHER-DIPLOMA', 'DIPLOMA', 'CERTIFICATE', 'SECONDARY', 'PRIMARY', 'NONE'])
export const educationTypeEnum = pgEnum("educationTypeEnum", ['academic', 'professional', 'training'])
export const employeeCategory = pgEnum("employeeCategory", ['UNIONISABLE', 'MANAGEMENT', 'NON-UNIONISABLE', 'WORKFLOOR', 'CONTRACTOR'])
export const employeeStatus = pgEnum("employeeStatus", ['ACTIVE', 'TERMINATED', 'RESIGNED', 'RETIRED', 'DECEASED'])
export const employmentType = pgEnum("employmentType", ['PERMANENT', 'CONTRACT', 'CASUAL', 'INTERN', 'NO CONTRACT'])
export const factorStatus = pgEnum("factor_status", ['verified', 'unverified'])
export const factorType = pgEnum("factor_type", ['webauthn', 'totp'])
export const genderEnum = pgEnum("genderEnum", ['MALE', 'FEMALE'])
export const healthSafetyInjuryEnum = pgEnum("healthSafetyInjuryEnum", ['NOT-FILED', 'PENDING', 'REJECTED', 'CLOSED', 'MINOR', 'MAJOR'])
export const healthSafetyStatusEnum = pgEnum("healthSafetyStatusEnum", ['NOT-FILED', 'PENDING', 'REJECTED', 'CLOSED'])
export const keyStatus = pgEnum("key_status", ['expired', 'invalid', 'valid', 'default'])
export const keyType = pgEnum("key_type", ['stream_xchacha20', 'secretstream', 'secretbox', 'kdf', 'generichash', 'shorthash', 'auth', 'hmacsha256', 'hmacsha512', 'aead-det', 'aead-ietf'])
export const leadStatusEnum = pgEnum("leadStatusEnum", ['new', 'contacted', 'nurturing', 'qualified', 'unqualified', 'lost'])
export const leaveStatusEnum = pgEnum("leaveStatusEnum", ['PENDING', 'APPROVED', 'REJECTED', 'CANCELED'])
export const maritalStatusEnum = pgEnum("maritalStatusEnum", ['SINGLE', 'MARRIED', 'DIVORCED', 'WIDOWED'])
export const opportunityStageEnum = pgEnum("opportunityStageEnum", ['prospecting', 'qualification', 'propasal', 'negotiation', 'won', 'lost'])
export const priorityEnum = pgEnum("priorityEnum", ['low', 'medium', 'high'])
export const quotationStatusEnum = pgEnum("quotationStatusEnum", ['draft', 'sent', 'accepted', 'declined', 'expired'])
export const salutationEnum = pgEnum("salutationEnum", ['mr', 'mrs', 'ms', 'dr', 'sir', 'prof', 'other'])
export const stockMovementTypeEnum = pgEnum("stockMovementTypeEnum", ['GRN', 'ISSUE', 'TRANSFER', 'CONVERSION', 'CONVERSION_IN', 'CONVERSION_OUT', 'OPENING_BAL'])
export const supportTicketEnum = pgEnum("supportTicketEnum", ['open', 'in progress', 'escalated', 'resolved', 'closed'])
export const userRoleEnum = pgEnum("userRoleEnum", ['STANDARD USER', 'ADMIN', 'SUPER ADMIN'])
export const vatTypeEnum = pgEnum("vatTypeEnum", ['NONE', 'EXCLUSIVE', 'INCLUSIVE'])
export const vehicleStatusEnum = pgEnum("vehicleStatusEnum", ['ACTIVE', 'GROUNDED', 'SOLD'])
export const workfloorType = pgEnum("workfloorType", ['UNIONISABLE', 'NON-UNIONISABLE'])
...
export const appraisalHeader = pgTable("appraisal_header", {
	...
	appraisalStatus: appraisalStatusEnum("appraisal_status").default('PENDING').notNull(),
...
export const disciplinaryCases = pgTable("disciplinary_cases", {
	...
	remarks: text().notNull(),
	caseStatus: disciplinaryCaseStatusEnum("case_status").default('REPORTED').notNull(),
...
export const employeeUsers = pgTable("employee_users", {
	...
	password: text(),
	employeeType: employeeCategory("employee_type").default('MANAGEMENT').notNull(),
	email: text(),
	image: text(),
	active: boolean().default(true).notNull(),
	...
	uniqueIndex("employee_user_contact_idx").using("btree", table.contact.asc().nullsLast().op("text_ops")),
	uniqueIndex("employee_user_id_number_idx").using("btree", table.idNumber.asc().nullsLast().op("text_ops")),
	index("employee_user_name_idx").using("btree", table.name.asc().nullsLast().op("text_ops")),
...
export const employees = pgTable("employees", {
	...
	index("employee_othernames_idx").using("btree", table.otherNames.asc().nullsLast().op("text_ops")),
	index("employee_surname_idx").using("btree", table.surname.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const healthSafety = pgTable("health_safety", {
	...
	applicationStatus: healthSafetyStatusEnum("application_status").default('NOT-FILED').notNull(),
...
export const jobcardTasks = pgTable("jobcard_tasks", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	...
	index("jobard_tasks_idx").using("btree", table.jobcardNo.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.departmentId],
			foreignColumns: [departments.id],
...
export const jobcards = pgTable("jobcards", {
	...
	unique("jobcards_jobcard_no_unique").on(table.jobcardNo),
]);
...
export const opportunitiesFiles = pgTable("opportunities_files", {
...
export const projects = pgTable("projects", {
	...
	index("project_name_idx").using("btree", table.projectName.asc().nullsLast().op("text_ops")),
]);
...
export const ordersHeader = pgTable("orders_header", {
	...
	displayOdometerReadingsOnPrint: boolean("display_odometer_readings_on_print").default(false),
...
export const quotationsHeader = pgTable("quotations_header", {
	id: uuid().defaultRandom().primaryKey().notNull(),
...
export const saleAccounts = pgTable("sale_accounts", {
	id: uuid().defaultRandom().primaryKey().notNull(),
...
export const quotationsItems = pgTable("quotations_items", {
	id: uuid().defaultRandom().primaryKey().notNull(),
...
export const roles = pgTable("roles", {
	...
	defaultPagePath: varchar("default_page_path", { length: 255 }).default('/dashboard').notNull(),
});
...
export const products = pgTable("products", {
	...
	index("product_name_idx").using("btree", table.productName.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const salesSupportTickets = pgTable("sales_support_tickets", {
	id: varchar().primaryKey().notNull(),
...
export const salesOrdersHeader = pgTable("sales_orders_header", {
	id: integer().primaryKey().notNull(),
...
export const vendors = pgTable("vendors", {
	...
	index("vendor_name_idx").using("btree", table.vendorName.asc().nullsLast().op("text_ops")),
]);
...
export const users = pgTable("users", {
	...
	promptPasswordChange: boolean("prompt_password_change").default(false),
	resetToken: text("reset_token"),
	hasAdminPriviledges: boolean("has_admin_priviledges").default(false).notNull(),
}, (table) => [
	uniqueIndex("contact_idx").using("btree", table.contact.asc().nullsLast().op("text_ops")),
	index("user_name_idx").using("btree", table.name.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.role],
...
export const appraisalDetails = pgTable("appraisal_details", {
	id: serial().primaryKey().notNull(),
	headerId: integer("header_id").notNull(),
	kpi: varchar().notNull(),
	objective: text().notNull(),
...
export const salesOrdersDetails = pgTable("sales_orders_details", {
	id: serial().primaryKey().notNull(),
	headerId: integer("header_id").notNull(),
...
export const disciplinaryCasesDocuments = pgTable("disciplinary_cases_documents", {
	...
	primaryKey({ columns: [table.caseId, table.uploadedUrl], name: "disciplinary_cases_documents_case_id_uploaded_url_pk"}),
]);

export const disciplinaryCasesPersonnel = pgTable("disciplinary_cases_personnel", {
	staffId: integer("staff_id").notNull(),
	caseId: text("case_id").notNull(),
}, (table) => [
	foreignKey({
	...
	primaryKey({ columns: [table.staffId, table.caseId], name: "disciplinary_cases_personnel_case_id_staff_id_pk"}),
]);

export const healthSafetyDocuments = pgTable("health_safety_documents", {
	caseId: text("case_id").notNull(),
	uploadedUrl: text("uploaded_url").notNull(),
}, (table) => [
	foreignKey({
	...
	primaryKey({ columns: [table.caseId, table.uploadedUrl], name: "health_safety_documents_case_id_uploaded_url_pk"}),
]);
...
export const userRoles = pgTable("user_roles", {
	...
	primaryKey({ columns: [table.userId, table.roleId], name: "user_roles_role_id_user_id_pk"}),
]);
