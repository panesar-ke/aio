...
import {
  generateOrderUrl,
  sendOrderEmailAction as sendOrderEmail,
...
export function OrderView() {
  ...
  const { data } = useSuspenseQuery(purchaseOrdersQueryOptions.order(orderId))

  const queryClient = useQueryClient()
  const { mutate, isPending } = useMutation({
    mutationFn: async (values: OrderData) => {
      return await generateOrderUrl({ data: { values, orderId } })
    },
    onSuccess: (res) => {
      if (res.error) {
      ...
      queryClient.invalidateQueries({
        queryKey: ['purchase_orders', orderId],
      })
    },
    onError: (error) => {
  ...
  const { mutate: emailMutate, isPending: isEmailPending } = useMutation({
    mutationFn: async (values: string) => {
      return await sendOrderEmail({ data: values })
    },
    onSuccess: (res) => {
      if (res.error) {
        toast.error(res.message)
      ...
      {!data.fileUrl ? (
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            size="lg"
            variant="secondary"
            onClick={() => emailMutate(orderId)}
            disabled={isEmailPending || isPending}
          >
            {isEmailPending ? (
